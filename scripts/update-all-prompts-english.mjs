...
async function main() {
  ...
  try {
    // 1. 管理者のFirebase UIDを取得
    console.log("\n管理者のFirebase UIDを取得中...");
    const allUsers = await client.query("promptItems_admin:getAllUsers");
    const adminUser = allUsers.find(user => user.email === ADMIN_EMAIL);

    if (!adminUser) {
      throw new Error(`管理者ユーザー (${ADMIN_EMAIL}) が見つかりません`);
    }

    const ADMIN_UID = adminUser.externalId;
    console.log(`✅ 管理者UID取得成功: ${ADMIN_UID}`);
    ...
    console.log(`合計: ${reiPrompts.length}件`);
